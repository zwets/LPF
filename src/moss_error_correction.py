...
# Command tool for performing error correction of a Pathogen Real Time Typer database system.
# In the case of isolate X having been wrongly grouped under reference Y, because reference Z did not exist yet, we might want to reanalyse X.
# Output results:
#   - X stays under Y
#   - X is moved to Z

# Shoulder either have a direct reference target, or a general screening based on a log file.

# Direct analysis:
#   - for selected reference:
#       - from refence_distmatrix, select min,min+1, max-1 and max isolate as set
#       - distance matrix of set X references to see what the best match is.
...
def prrtAnalysis(input_type, inputlist, masking_scheme, prune_distance, bc, configname, multi_threading, exepath, output_name, i):
    ...
    oneliner += " -configname {}".format(configname)
    oneliner += " -exepath {}".format(exepath)
    oneliner += " -prune_distance {}".format(prune_distance)
    oneliner += " -o {}{}".format(output_name, i)
    os.system(oneliner)

def main(input_type, masking_scheme, prune_distance, bc, configname, multi_threading, output_name, exepath, parallel_input, jobs):
    #LAV OMFATTENDE ERROR CHECK SÅ FRONT-END IKKE FEJLER
    cmd = "mkdir " + configname + "multiSampleAnalysisReports/{}".format(output_name)
    os.system(cmd)
    cmd = "touch " + configname + "multiSampleAnalysisReports/{}/stdout".format(output_name)
    os.system(cmd)
    cmd = "touch " + configname + "multiSampleAnalysisReports/{}/stderr".format(output_name)
    os.system(cmd)
    ...
    Parallel(n_jobs=jobs)(delayed(prrtAnalysis)(input_type, inputlist, masking_scheme, prune_distance, bc, configname, multi_threading, exepath, output_name, i) for i in range(len(inputlist)))
    print ("Analysis complete")

if __name__== "__main__":
  main(args.input_type, args.masking_scheme, args.prune_distance, args.bc, args.configname, args.multi_threading, args.output_name, args.exepath, args.parallel_input, args.jobs)
