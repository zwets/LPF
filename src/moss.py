...
def moss_pipeline(seqType, masking_scheme, prune_distance, bc,
    ...
    if len(moss.loadFiles("{}datafiles/isolatefiles/{}/".format(db_dir, refname))) > 1:
        ...
        moss_sql.update_isolate_table(entryid, header_text, samplename, str(datetime.datetime.now())[0:-7], plasmid_string.replace("'", "''"), allresgenes.replace(", ", ",").replace("'", "''"), virulence_string.replace("'", "''"))
...
def main():
    moss_pipeline(args.seqType, args.masking_scheme, args.prune_distance, args.bc, args.db_dir, args.multi_threading, args.exepath, args.laptop, args.metadata, args.metadata_headers)
